...
if ($Help) {
    ...
    Write-Host ""
    Write-Host "用法:"
    Write-Host "  .\install-codex.ps1           正常安装"
    Write-Host "  .\install-codex.ps1 -Help     显示帮助"
    Write-Host "  .\install-codex.ps1 -DryRun   预览步骤"
    Write-Host ""
    Write-Host "跳过确认:"
    ...
    Write-Host "  浏览器会自动弹出 ChatGPT 登录页"
    Write-Host ""
    Write-Host "常见失败原因:"
    Write-Host "  1. Node.js 版本不足 → 脚本会自动安装 v22+"
    Write-Host "  2. npm 网络问题 → 国内用户可尝试切换网络"
    Write-Host "  3. 安装后找不到 codex → 关掉 PowerShell 重开"
    Write-Host ""
    exit 0
}
...
Write-Host "  本脚本将安装 Codex" -ForegroundColor Cyan
Write-Host "  需要 ChatGPT 账号或 OpenAI API Key" -ForegroundColor Cyan
Write-Host ""
...
if ($DryRun) {
    ...
    if ($needNode) {
        if (-not (Confirm-Action "即将安装 Node.js v22+（系统级软件），是否继续？")) {
            Write-Host "  [FAIL] Node.js 是 Codex 的必要依赖，无法跳过" -ForegroundColor Red
            exit 1
        }
...
# ---- 安装 Codex ----
Write-Host "--- 安装 Codex ---" -ForegroundColor Blue

if (-not $DryRun -and (Get-Command codex -ErrorAction SilentlyContinue)) {
...
if ($DryRun) {
    ...
    Write-Host "  [OK] Codex 安装验证通过" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "  或使用 API Key:" -ForegroundColor Green

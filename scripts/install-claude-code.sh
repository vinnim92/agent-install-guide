...
show_help() {
    ...
    echo "  curl -fsSL \"https://cdn.jsdelivr.net/gh/vinnim92/agent-install-guide@v3.0.6/scripts/install-claude-code.sh\" | AGENT_INSTALL_YES=1 bash"
    echo ""
    echo "如果你想手动确认，请先下载脚本再运行："
    echo "  curl -fsSL \"https://cdn.jsdelivr.net/gh/vinnim92/agent-install-guide@v3.0.6/scripts/install-claude-code.sh\" -o install-claude-code.sh"
    ...
    if run_cmd "curl -fsSL ${OFFICIAL_URL} | bash 2>/dev/null"; then
        ...
        if command_exists npm; then
            run_cmd "npm install -g @anthropic-ai/claude-code@latest 2>/dev/null" && print_success "npm 安装成功" || {
                print_error "安装失败"
                echo ""
                echo "  排查: 访问 https://cdn.jsdelivr.net/gh/vinnim92/agent-install-guide@v3.0.6/docs/support.html"
                exit 1
            }
        else

...
# ---- 禁止 @main 未版本化 URL ----
echo ""
echo "--- 检查: 禁止 @main 未版本化 URL（应使用 @v3.1.0 等版本标签） ---"
...
if [ -n "$AT_MAIN_HITS" ]; then
    check_fail "发现 @main 未版本化 URL（应固定为 @v3.1.0 等版本标签）"
...
if [ -n "$V3_OLDER_DELIVERY" ]; then
    check_fail "发现活动旧版本交付命令（应使用 @v3.1.0）"
    echo "$V3_OLDER_DELIVERY" | while read line; do echo "       $line"; done
else
    check_pass "正式交付命令均使用 @v3.1.0"
fi

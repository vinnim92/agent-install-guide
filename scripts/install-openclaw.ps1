    ...
    if ($DryRun) {
        ...
        if ($hasNode) {
            ...
            if ($majorVer -gt 22 -or ($majorVer -eq 22 -and $minorVer -ge 19)) {
                ...
                Write-Host "    [!] Node.js v$nodeVer 版本较低，将自动安装（推荐 v24+）" -ForegroundColor Yellow
            }
        } else {
            Write-Host "    [!] Node.js 未安装，将自动安装（推荐 v24+）" -ForegroundColor Yellow
        }
    }
...
function Install-NodeIfNeeded {
    if ($DryRun) {
        Write-DryRun "检查 node 版本 (需要 >= 22.19)，如需安装则 winget install OpenJS.NodeJS.LTS"
        return
    }
    ...
    if ($needNode) {
        if (-not (Confirm-Action "即将安装 Node.js（推荐 v24+），是否继续？")) {
            Write-Host "  [FAIL] Node.js 是 $AgentName 的必要依赖" -ForegroundColor Red
            exit 1
        }
...
function Start-Install {
    ...
    if ($installed) {
        ...
        Write-Host "  4. 故障排查: https://vinnim92.github.io/agent-install-guide/docs/support.html" -ForegroundColor Yellow
        exit 1
    }
}

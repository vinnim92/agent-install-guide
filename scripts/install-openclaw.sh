...
show_help() {
    ...
    echo ""
    echo "用法:"
    echo "  bash install-openclaw.sh           正常安装"
    echo "  bash install-openclaw.sh --help    显示帮助"
    echo "  bash install-openclaw.sh --dry-run 预览步骤（不安装）"
    echo ""
    echo "跳过确认:"
...
check_linux_build_deps() {
    if [ "$OS_TYPE" = "linux" ]; then
        ...
        if [ "$DRY_RUN" = true ]; then
            print_dryrun "检查 gcc g++ make 是否安装"
            return 0
        fi
        local missing=""
...
install_openclaw() {
    ...
    if command_exists openclaw; then
        print_success "OpenClaw 已安装 ($(openclaw --version 2>/dev/null | head -1 || echo '未知版本'))"
        return 0
    fi
...
verify_installation() {
    ...
    if command_exists openclaw; then
        ...
        openclaw --version 2>/dev/null | head -1 | while read -r v; do echo -e "  ${v}"; done
    else
...
echo -e "  本脚本将安装 ${BOLD}OpenClaw${NC}"
echo -e "  内置免费模型，无需 API Key"
echo ""
...
echo ""
print_success "OpenClaw 安装验证通过"
echo ""
echo -e "${GREEN}第一次启动:${NC}"
echo "  在终端输入 openclaw 进入交互式对话"
echo "  或输入 openclaw dashboard 打开 Web 控制台"
echo ""
echo -e "  推荐初始化:"
